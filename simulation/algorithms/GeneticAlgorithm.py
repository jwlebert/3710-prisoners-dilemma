...
class GeneticIndividual:  # individual agent
    ...
    def fitness(self):
        if self.__fitness is None:
            tournament = OptimizedTournament(
                self.chromosome, self.memory_depth, rounds=50
            )
...
class GeneticAlgorithm(OptimizationAlgorithm):  # population
    def __init__(
        self, pop_size: int = 20, mutation_rate: float = 0.001, memory_depth: int = 3
    ):
        super().__init__(name="GeneticAlgorithms")

        self.population: List[GeneticIndividual] = []
        self.pop_size: int = pop_size  # best when even
        self.mutation_rate: float = mutation_rate
        self.memory_depth: int = memory_depth
        self.gene_bit_len: int = 2 ** (
            2 * memory_depth
        )  # number of bits in chromosomes
